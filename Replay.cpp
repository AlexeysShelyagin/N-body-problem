...
Replay::Replay(std::string filename) {
    ...
    frame_num = (world_file.size() / sizeof(double) - 2) / (4 * body_num + 3);
    dt_list.resize(frame_num);

    double last_t = 0, T, dt_max = 0, end_t = 0;
    for(int i = 0; i < frame_num; i++){
        T = world_file.read_double(sizeof(double) * (2 + (4 * body_num + 3) * i));
        dt_list[i] = T - last_t;
        last_t = T;
        end_t += dt_list[i];
...
bool Replay::load_frame(int _frame) {
    frame = _frame;
    if(frame < frame_num) world = world_file.read_world(body_num, sizeof(double) * (2 + (4 * body_num + 3) * frame));
